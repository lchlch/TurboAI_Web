...
class Store {
  ...
  fetchRecords = () => {
    ...
    return http.get('/api/v1/server/host/canBuyList')
      .then(res => {
        // const tmp = {};
        ...
        // this.rawRecords.map(item => tmp[item.hostId] = item);
        // this.idMap = tmp;
      })
    ...
    return http.get('/api/v1/server/host/canBuyList')
      .then((res1) => {
        this.rawRecords = res1.map(item => {
          item.host = item.hostId.toString()
          return item;
        });
        // this.rawRecords.map(item => this.idMap[item.hostId] = item);
